...
«Ох, деточка, помню-помню, как эти машины умели. Скажу: „Мама мыла…“ — и она уже знает, что дальше
„раму“. А „мал золотник, да…“ — и выберет „дорог“, не сомневайся, умничка. Не потому, что понимает,
а потому что насмотрелась текстов за свои годы — как я на завод. Глянет на твой текст и **предсказывает
следующий кусочек** — за кусочком, предложение за предложением. Каждому варианту свой процент — берёт
самый жирный.
...
**Генеративный ИИ** — ИИ, который умеет **создавать** новое: текст, изображения, код. Не искать и повторять,
а генерировать. **LLM (большая языковая модель)** — вид генеративного ИИ, обученный на огромном количестве
текстов: писать и резюмировать, отвечать на вопросы, помогать с кодом, структурировать данные.

Только, деточка, без б — проверять всё равно надо.
...
«Смотри, деточка, как мы с ними разговариваем. Машине дают **наряд-заказ** — задание, что сделать.
Для модели это **промпт (запрос)**: твой текст-вход. А результат работы — **завершение (completion)**:
текст, который машина выдала.

И вот важное: машина не читает заказ назубок, а подбрасывает монетку. Поэтому ответ **не детерминирован** —
при одном и том же запросе он может отличаться. Разброс регулирует **температура**: низкая — стабильнее,
высокая — больше фантазии.

⚠️ И ещё: машина может **выдумать** — это называют галлюцинациями. И твои файлы она не видит, пока ты их
не прикрепишь. Всё проверяем. Подробнее — [как ловить галлюцинации и проверять ответы](@galyucinacii-i-proverka).

Про токены — из каких «кирпичиков» собран текст — разберём в занятии 2.»
```
...
- **Содержание:**
  ...
  - `description`: «Разберёшься в ИИ лучше всех — только не верь ему на слово. Погнали знакомиться
    с Василичем-5.7.»
...
- **Содержание:**
  ...
  - `task` (Markdown): «Поздоровайся с Василичем-5.7 и расспроси её: кто она, что умеет, чем может помочь заводу. Попробуй задать ей вопрос про завод — например, „с чего начать, чтобы вернуть производство?“. Заметь: даже на один и тот же вопрос ответы могут отличаться — это нормально.»
...
- **Содержание:**
  - `task` (Markdown): «Сформулируй **три вопроса про завод** и задай их Василичу-5.7. Примеры: „Что умеет древний ИИ?“, „Где у нас хранятся данные?“, „С чего начать восстановление производства?“. Вопросы должны быть по делу. После ответов нажми „Проверить“.»
  - `systemPrompt`: текст из `files/system-prompt-vasylich-57.md`
  - `aiName`: «Василич-5.7»
  - `model`: `openrouter/free`
...
- **Содержание:**
  - `task` (Markdown): «Распроси Василича-5.7 про заводские древности: какие древние ИИ и машины есть,
    какие данные лежат в архиве (таблицы, накладные, логи). Составь **рапорт на имя Василича
    (около 300 слов)**: что есть в древних завалах железа и данных и как это поможет вернуть производство.
    Структура: **что в завалах** (древние машины и ИИ, данные архива), **как это поднимет производство**
    (что пустить в дело первым), **проверка** (как ты проверяешь, что данные и ответы ИИ не врут).
    Черновик можно составить с помощью Василича-5.7, но рапорт должен быть твоим. По готовности нажми
    „Проверить“.»
  - `systemPrompt`: текст из `files/system-prompt-vasylich-57.md`
  - `aiName`: «Василич-5.7»
