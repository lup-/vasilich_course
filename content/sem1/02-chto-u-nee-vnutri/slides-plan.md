...
> «Ну чё, молодежь, ожил черт, гляжу. Теперь главное — раскурить его мозги: из чего он сделан,
> чем один черт от другого отличается и какого под какой цех ставить. Разберёшься — решишь,
> какая машина на какое дежурство годится. Только сразу скажу: они врут, все. С этим потом.» — Прораб Василич

**Задача квеста:** разобрать «мозги» древнего ИИ: из чего он сделан (токены, служебные токены, окно
контекста, вероятностная генерация), чем модели отличаются друг от друга, и обоснованно выбрать
«модель под цех».

**Итог занятия:** студент понимает, как LLM «видит» текст (токены → числа), знает про служебные токены
...
| 6 | `image` | Мем «Текст превращается в числа» | После токенизации: разрядка после блока текста, токенизатор «переводит» слова в числа (мем, человек проверяет) |
| 7 | `rich_text` | «Служебные токены» | От Алисы: штампы на накладных (~100 слов): `<\|im_start\|>`, `<\|im_sep\|>`, `<\|im_end\|>` + мини-пример ChatML + приватность |
| 8 | `rich_text` | «Окно контекста и случайность» | От Алисы (~120 слов): окно-память, «взвешенная монетка», температура; инструменты bbycroft (ссылка) и TokenProbe (слайд 9) |
| 9 | `embed` | «TokenProbe: живой выбор токена» | Инструмент в слайде: вероятности следующего токена, температура |
| 10 | `rich_text` | «Что за цифры в имени: 20B и 120B» | От Алисы (~100 слов): B = миллиарды параметров; 20B — маленькая/быстрая/дешёвая, 120B — большая/мощная/дорогая; ссылка на доп. блок |
| 11 | `ai_comparison` | «Одна задача — две модели» | Два чата рядом, одна и та же задача, одинаковый промпт Василича-5.7, разные модели (gpt-oss-20b vs nemotron-3-super-120b) |
| 12 | `image` | Мем «Большую машину на рутину не гоняем» | Перед выбором модели под цех: экономика автоматизации (мем, человек проверяет) |
| 13 | `ai_task_chat` | «Выбери модель под цех» | Контрольная точка: обоснованный выбор модели под 3 заводские работы (качество/скорость/цена), `checkPrompt` |
| 14 | `quiz` | «Что верно про токены» | 1 правильный вариант (по лекциям 01–02) |
| 15 | `open_question` | «Как называется кусочек текста?» | Открытый вопрос: «токен / token» |
| 16 | `image_group` | Комикс-развязка «Модель под цех» | 4 панели: доклад выбора, ворчание про расходы, хук на занятие 3 («не халтурить») |
...
«Так вот, деточка, слушай, как машина читает. Не буквами, а **токенами** — кусочками текста:
где слово целиком, где хвостик слова, где запятая с пробелом. Каждому кусочку — свой номерок-число,
**ID**. А чтобы сравнивать кусочки по смыслу, машина превращает каждый в **эмбеддинг** — длинный ряд
чисел. Похожие по смыслу токены живут рядом. Как это выглядит — смотри ролик, деточка.»
```
...
````markdown
«То, что я по-бабушкиному объяснила, деточка, — теперь по-взрослому, без сказок.»
...
### 6. image — Мем «Текст превращается в числа»
...
«Помню, деточка, как на старых накладных штампы ставили: „Принято“, „Отгружено“, „Конец партии“.
Вот и разговор с машиной — та же накладная: чат-приложение ставит вокруг сообщений **служебные токены**,
чтобы машина понимала, кто говорит и где конец.

```text
<|im_start|>system … <|im_end|>
<|im_start|>user Вопрос… <|im_sep|>
<|im_start|>assistant Ответ… <|im_end|>
```

`<|im_start|>` — начало сообщения, `<|im_sep|>` — разделитель, `<|im_end|>` — конец. Ты их не печатаешь —
их подставляет приложение. Увидишь в tiktokenizer (шаблон `gpt-4o`) и в логах.

⚠️ Вместе с промптом служебные токены уходят в облако: секреты и API-ключи в чат не вставляем.

Полная справка — [по служебным токенам](@sluzhebnye-tokeny-podrobnee).»
````

### 8. rich_text — «Окно контекста и случайность»
...
«Так, деточка, теперь про память и про „монетку“. Голова у машины маленькая: сколько токенов влезло
в **окно контекста**, столько она и видит. Что за окном — „забыто“. Важное клади в начало запроса
или во вложение.

А отвечает машина не с листа. Она подбрасывает **взвешенную монетку**: смотрит на контекст, выдаёт
вероятности для следующего токена и выбирает. Поэтому ответы меняются (вспомни занятие 1).

Разброс регулирует **температура**: низкая — „сухой“ стабильный ответ, высокая — больше хаоса,
средняя (0.7–1.0) — баланс.

Попробуй сам вживую — в следующем слайде: TokenProbe покажет, как модель выбирает следующий токен
по вероятностям, и позволит покрутить температуру. Вся „начинка“ модели — на bbycroft.net/llm
(ссылка в `links.md`).
...
### 9. embed — «TokenProbe: живой выбор токена»

- **Назначение:** встроенный инструмент (вместо ссылки из слайда 8): студент видит вероятности
...
- **Содержание:**
  ...
  - `url`: `https://tokenprobe.cs.columbia.edu`
...
### 10. rich_text — «Что за цифры в имени: 20B и 120B»

- **Назначение:** перед сравнением моделей (слайд 11) расшифровать, что означают цифры в именах моделей
...
«Вижу, деточка, ты на таблички смотришь: 20B, 120B. Это как размер бригады: **B** — миллиард,
а цифра — сколько у машины **параметров**, настроек, которые она набрала при обучении. Чем больше
бригада, тем больше она умеет — но и кормить её дороже, и работает она медленнее.

20B — маленькая бригада: отвечает быстро, стоит копейки, но в сложном путается. 120B — большая:
точной и полной отвечает, зато медленнее и в проде дороже. На занятии обе бесплатные, а в жизни
за большую платят — поэтому под задачу ставят подходящую, а не самую большую.

Подробнее — [что значат цифры в имени модели](@razmer-modelej).»
```

### 11. ai_comparison — «Одна задача — две модели»
...
- **Содержание:**
  ...
  - **Примечание (экономика):** на бесплатном тарифе OpenRouter обе машины бесплатны, а «дорогая №2» —
    ...
    выбора вернётся в экономику автоматизации (слайд 12).
  - `mergeInput`: true
...
### 12. image — Мем «Большую машину на рутину не гоняем»
...
### 13. ai_task_chat — «Выбери модель под цех»
...
- **Содержание:**
  - `task` (Markdown): «На заводе три работы: (1) отвечать на рутинные вопросы смены по инструкции,
    (2) считать расход компонентов по накладным, (3) писать длинные инструкции и регламенты для новых линий.
    Прогони одну-две работы на обеих машинах (слайд 11) и реши: **какую машину под какой цех поставить
    и почему**. Учти качество, скорость и цену: машина №1 маленькая и дешёвая, №2 — большая и дорогая
    (дорогую на рутину не гоняют). Критерии выбора — [в справке](@landshaft-modelej). Запиши выбор
    и объяснение и нажми „Проверить“.»
...
### 15. open_question — «Как называется кусочек текста?»
...
### 16. image_group — Комикс-развязка «Модель под цех»
