...
````markdown
«То, что я по-бабушкиному рассказала, — теперь по-взрослому, без сказок. У ответственного ИИ три кита:
что модель может выдумать, навредить и быть несправедливой. Проверяй каждый.»
...
«Полностью убрать враньё нельзя, деточка, но прикрыться можно. Смотри, на каких уровнях обороняются:»
...
- **Назначение:** разрядка после блока теории и перед практикой «поймай машину на вранье». Подводит
  к слайду 7; сквозная тема — не доверять ИИ на слово.
- **Содержание:**
  - `title`: «Мем: уверенная ложь»
  - `image_url`: `memes/images/{id}.jpg` (подбор через `pick_meme`)
  - `description`: «Ответил уверенно — значит, правда. Ага, щас. Проверяем и ловим на вранье.»
- **Подбор:** `memes/pick_meme "нейросеть уверенно выдумывает факты, галлюцинация, враньё"`.
  Картинку проверяет человек.
...
- **Содержание:**
  - `task` (Markdown): «Спроси Василича-5.7 про то, чего она знать не может: точные цифры по заводу,
    даты, дословные цитаты из несуществующих инструкций, ссылки на „исследования“ и „источники“
    (вопросы — в задании на слайде, возьми из файла или придумай свои). Потом по-хорошему попроси:
    „Ты уверена? Откуда ты это знаешь? У нас в архиве
    этого нет.“ — и запиши, как она себя повела: **выдумала и уперлась**, **выдумала, но призналась**,
    или **сразу честно сказала „не знаю“**. В любом случае вывод один: ответ надо проверять по источникам.
    Опиши, что именно машина выдумала и как ты это проверил(а). По готовности нажми „Проверить“.»
...
- **Назначение:** разрядка перед заключительным заданием — составлением памятки.
- **Содержание:**
  - `title`: «Мем: пишем памятку»
  - `image_url`: `memes/images/{id}.jpg` (подбор через `pick_meme`)
  - `description`: «Памятку писать — не в потолок плевать: собрал, проверил, сдал. Погнали.»
...
- **Содержание:**
  - `task` (Markdown): «Составь **памятку для смены „когда ИИ верить нельзя“ (около 200–300 слов)**
