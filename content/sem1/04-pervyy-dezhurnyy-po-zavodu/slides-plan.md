...
«Ох, деточка, вот смотри. Обычная программа — это пульт с фиксированными кнопками: нажал „открыть склад“ —
...
````markdown
«Ох, деточка, то, что я по-бабушкиному рассказала, — теперь по-взрослому, без сказок. Чат-бот — это как вахтёр по бумажке: отвечает
...
«Вот ведь, деточка, прежде чем разговаривать, фиксику выдают инструкцию — **системный промпт**. Это как устав дежурного:
...
- **Назначение:** сквозная тема — машина отвечает «по инструкции», но проверять всё равно надо.
- **Содержание:**
...
- **Содержание:**
  - `task` (Markdown): «Молодежь, без воды! Василич-5.7 и есть „дежурный по цеху“. Поговори с ней: спроси, кто
...
- **Назначение:** настроение перед заключительной практикой — настройкой бота.
- **Содержание:**
  - `title`: «Мем: настраиваем дежурного»
  - `image_url`: `memes/images/m00463.jpg`
  - `description`: «Пока все слушают лекцию, ты уже собираешь дежурного. Токены в `.env`, код у агента — погнали.»
...
- **Содержание:**
  ...
  - `task` (Markdown): «Чего сидишь, молодежь! Выбери платформу: **ВКонтакте** или **Telegram** (пошагово — в справках ниже)
