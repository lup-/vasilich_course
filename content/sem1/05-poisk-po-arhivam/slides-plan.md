...
| 7 | `resources` | «Корпус документов завода» | Скачивание: `dokumenty-arkhiva.zip` (6 документов) + памятка по сборке поиска |
| 8 | `video` | «RAG: дежурный ищет в архиве» | Manim ~74 с: вопрос → вектор → поиск в индексе → топ-3 чанка → контекст+вопрос → модель → ответ по документам |
| 9 | `rich_text` | «RAG: ответ по документам» | Формально (лекция 15): база знаний, чанки, извлечение, расширенная генерация; зачем (актуальность, меньше выдумок, дешевле); Mermaid-flowchart |
| 10 | `rich_text` | «RAG в готовых сервисах» | Тот же RAG без кода: NotebookLM, Open Notebook — загрузил документы и спрашиваешь; ответ с указанием источника; приватность: документы уходят в облако сервиса |
| 11 | `quiz` | «Что даёт RAG» | 3 варианта; верно: «модель отвечает по проверяемым документам из базы знаний — меньше выдумок» |
| 12 | `image` | Мем «поиск по смыслу» | Разрядка перед практикой (человек проверяет картинку) |
| 13 | `rich_text` | «Собираем поиск по архиву» | Что соберёт агент: чанки → эмбеддинги → индекс → RAG-ответ; сравнение с поиском по словам; секреты в `.env`; экономика токенов |
| 14 | `open_question` | «Собери поиск и проверь ответ» | Контрольная точка: студент собирает RAG через агента, запускает локально, задаёт контрольный вопрос; ответ авто-проверяется по эталону (склейка четырёх документов) |
| 15 | `image_group` | Комикс-развязка «Дежурный раскурил архив» | 4 панели: дежурный отвечает по документам, Василич принимает, хук на занятие 6 («этикетки рисовать умеет? кнопку жать?») |
...
- **Хук:** сцена 2 развязки (слайд 15) подводит к занятию 6 («Этикетки и кнопки»).
...
### 7. resources — «Корпус документов завода»
...
- **Содержание:**
  ...
  - `description` (Markdown): «Это архив завода — то, по чему будет искать дежурный. Скачай оба файла.
    ...
    Памятка — подсказка, как собирать поиск и проверять ответ. Всё понадобится на слайде 14.»
  - `files`:
...
### 8. video — «RAG: дежурный ищет в архиве»
...
### 9. rich_text — «RAG: ответ по документам»
...
### 10. rich_text — «RAG в готовых сервисах»
...
### 13. rich_text — «Собираем поиск по архиву»
...
### 14. open_question — «Собери поиск и проверь ответ»
...
- **Содержание:**
  ...
  - `markdown` (задание): «Собери поиск по архиву — RAG — и проверь его на контрольном вопросе.
    1. Скачай корпус документов завода со слайда 7 (`dokumenty-arkhiva.zip`) и распакуй его
...
### 15. image_group — Комикс-развязка «Дежурный раскурил архив»
...
слайд 9 (формальное объяснение RAG).
...
| `rag-i-vektornye-bazy` | 9, 11 | «RAG и векторные базы» |
| `servisy-rag-po-dokumentam` | 10 | «готовые сервисы по документам» |
| `kak-proverit-rag-otvet` | 14 | «как проверить задание» |
| `opencode-ustanovka` | 13, 14 | «как поставить opencode» |
| `razmer-modelej` | 14 | «что значат 120b и a12b в названии модели» |
