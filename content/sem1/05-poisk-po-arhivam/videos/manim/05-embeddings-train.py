# -*- coding: utf-8 -*-
"""Мама, откуда взялись эмбеддинги. Технический ролик Manim (ManimCE) для занятия 05.
...
Сюжет ролика:
    ...
    2) Обучение в пространстве смысла — «угадайка»: модель прячет один токен
       ...
       контекста (позитивное сэмплирование). Угадала — так и оставляем.
       Со 2-го шага — негативное сэмплирование: случайные «неправильные»
       токены отталкиваются от центра.
    3) Координаты точек складываются в кластеры по смыслу; в конце — 4
       явных кластера (СТИРКА/ХИМИЯ/РАСЧЁТ/УЧЁТ), облёт сетки и 8 главных
       токенов.
    4) Стрелка от начала координат к «ПОРОШОК» с координатами →
       «координаты этой точки — вектор — и называется эмбеддингом»;
       финал: для реального языка 3 числа мало, модели используют
       эмбеддинги из 384 измерений.
...
в 4 явных кластера (цветовая разметка в финале). Из 15 шагов модель ровно
2 раза угадывает (шаги 10 и 13).

Запуск:
    python -m manim render -ql -p --format mp4 --resolution 1920,1080 --fps 30 --seed 5 \
        05-embeddings-train.py Chisla
"""
...
class Chisla(ThreeDScene):
    ...
    def corpus_scene(self):
        ...
        hook = Text(
            ...
            "И ПОИГРАЕМ В ИГРУ.\n"
            "В КАЖДОМ ПРЕДЛОЖЕНИИ СПРЯЧЕМ ОДИН ИЗ ТОКЕНОВ —\n"
            "А МОДЕЛЬ БУДЕТ УГАДЫВАТЬ.\n"
            "ЦЕЛЬ ИГРЫ — ПОДОБРАТЬ ТОКЕНАМ\n"
            "ПРАВИЛЬНОЕ ОКРУЖЕНИЕ.",
            font="DejaVu Sans", font_size=26, weight=BOLD,
            t2c={
                "РАЗБРОСАЕМ ТОКЕНЫ ПО «КОМНАТЕ»": PHOSPHOR,
                "И ПОИГРАЕМ В ИГРУ.": PHOSPHOR,
                "В КАЖДОМ ПРЕДЛОЖЕНИИ СПРЯЧЕМ ОДИН ИЗ ТОКЕНОВ": BEIGE,
