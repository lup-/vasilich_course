...
мета-промты) и что такое вызов инструментов (tool calling) — как ИИ получает «руки»; сгенерировать
этикетки для позиций завода (порошок, сода, цемент), чтобы люди отличали мешки и продукция шла
в магазины с названиями, и через агента собрать скрипт, который через вызов инструментов узнаёт
реальные позиции со склада и генерирует по ним этикетки (задание можно сделать на платформе
или в локальном opencode).

**Итог занятия:** студент объясняет, как из текста получается картинка и что такое вызов инструментов;
сдаёт Василичу этикетку «ПОРОШОК-МАКС» (обязательная контрольная точка; «СОДА-ЭКСТРА» и «ЦЕМЕНТ-84» —
по желанию) и задачу «Кнопка „склад“ и этикетки» — скрипт, который через вызов инструментов возвращает
...
|---|-----|----------|--------------------|
| 1 | `image_group` | Комикс «Этикетки и кнопки» | 5 панелей: мешки без подписей, цемент в стиралку → Алиса про «машина рисует» и «кнопку-инструмент» → склад молчит → задача: позиции, этикетки и кнопки |
...
| 7 | `rich_text` | «Кнопка для машины» | Лекция 11: модель «из головы» не знает живых данных; вызов инструментов — «руки»; метафора «дежурный с кнопкой» |
| 8 | `video` | «Вызов инструментов: руки для модели» | Manim ~78 с: вопрос о позициях → модель решает вызвать «склад» → программа выполняет → ответ модели → без инструмента — «из головы» |
| 9 | `rich_text` | «Вызов инструментов: как ИИ получает руки» | Формально (лекция 11): tools, описание инструмента, цикл вызова; Mermaid-схема; токены и приватность |
| 10 | `quiz` | «Что такое вызов инструментов» | 3 варианта; верно: «модель решает вызвать инструмент, программе остаётся её выполнить и вернуть реальный результат» |
...
| 14 | `rich_text` | «Собираем кнопку „склад“» | Что соберёт агент: вызов инструментов через OpenRouter + этикетки для позиций в наличии; сравнение с ответом «из головы»; секреты в `.env`; экономика токенов |
| 15 | `ai_code_task` | «Кнопка „склад“ и этикетки» | Главная практика-контрольная точка: студент словами описывает агенту скрипт — вызов инструмента «склад» (контрольный вопрос «Какие позиции есть на складе?») + этикетки для позиций в наличии; делается на платформе или в локальном opencode; проверяет список позиций по файлу и каждую этикетку глазами |
...
- **Содержание:**
  ...
  - `description`: «Склад, утро. Мешки порошка, соды и цемента стоят в ряд — одинаковые, без подписей.
    ...
    рисовать по словам и „жать кнопку“ — вызывать инструмент. Склад молчит, живого списка позиций
...
- **Назначение:** первое понятие от имени Алисы (п.7 шаблона), по лекции 11 курса-01: модель отвечает
  «из головы» и не знает живых данных; вызов инструментов (tool calling) — «руки»: модель понимает,
  что нужен инструмент, и вызывает его. Одна мысль: «дежурный с кнопкой на столе».
- **Содержание** (`markdown`):
...
Чтобы дать машине руки, придумали **вызов инструментов**. Это как кнопки на пульте дежурного: на складе стоит
кнопка „СКЛАД“. Спросили про позиции — модель понимает: „нужна кнопка“, нажимает её (вызывает инструмент),
инструмент читает настоящие данные и возвращает результат — модель уже по нему отвечает.
...
Подробнее — [вызов инструментов (tool calling)](@vyzov-instrumentov).»
```

### 8. video — «Вызов инструментов: руки для модели»

- **Назначение:** наглядно показать механизм вызова инструментов после рассказа Алисы: вопрос → модель
  решает вызвать инструмент «склад» → программа выполняет (читает файл) → результат возвращается модели →
  ответ по реальным данным. Параллельный кадр: без инструмента модель «из головы» привирает. Технический ролик (Manim).
- **Содержание:**
  - `title`: «Вызов инструментов: руки для модели»
  - `video_url`: `videos/06-vyzov-instrumentov.mp4` (отрендеренный файл)
  - `video_type`: `direct`
  - `description`: «Технический ролик: смена спрашивает „КАКИЕ ПОЗИЦИИ ЕСТЬ НА СКЛАДЕ?“. Модель понимает,
    что нужен инструмент „СКЛАД“, и формирует вызов с аргументом (tovar = позиция). Программа выполняет
    инструмент — читает файл склада — и возвращает модели карточку позиции (есть ли в наличии). Модель
    проверяет позиции по одной и собирает ответ: на складе порошок, сода, цемент (сахара нет — остаток 0).
    Для сравнения: без инструмента та же модель отвечает „из головы“ — называет позиции как попало, красный
    крестик „ПРОВЕРЯЙ“. Вывод: инструмент — руки модели: вызвал → реальный результат → ответ по делу.
    Программа-сценарий: `videos/manim/06-vyzov-instrumentov.py` (см. `video-guide.md`).»
- **План ролика (примерный сценарий, ~76 с):** см. раздел «Видео: план ролика» ниже.

### 9. rich_text — «Вызов инструментов: как ИИ получает руки»

- **Назначение:** формальное объяснение ПОСЛЕ видео (п.7 шаблона), по лекции 11 курса-01: инструменты
  (tools), описание инструмента, цикл «модель решает → программа выполняет → результат модели»;
  Mermaid-схема; экономика токенов и приватность.
- **Содержание** (`markdown`):

````markdown
«Деточка, то, что я по-бабушкиному показала, — теперь по-взрослому, без сказок. **Вызов инструментов (tool calling)** — способ
дать модели доступ к внешним данным и действиям. Модель сама не выполняет код: она решает, какой
инструмент вызвать, и формирует аргументы, а реальный инструмент выполняет программа.

Как это устроено:

1. **Описываем инструмент**: имя, параметры, что она делает (например `sklad(tovar)` —
...
3. **Программа выполняет** инструмент и кладёт результат в диалог.
...
Два момента: описание инструментов уходит в контекст вместе с запросом — это **токены**, то есть деньги
(экономия — описывать инструменты коротко и только нужные). И **приватность**: в инструмент уходят данные из
твоего запроса — чужие данные и секреты в такие вызовы не шлём (занятие 3).

Подробнее — [вызов инструментов (tool calling)](@vyzov-instrumentov).»
````

### 10. quiz — «Что такое вызов инструментов»

- **Назначение:** контрольная точка по теории (по лекции 11 курса-01). Материал — `files/quiz-vyzov-instrumentov.md`.
- **Содержание:**
  - `question`: «Что такое вызов инструментов (tool calling)?»
  - `markdown`: «Выбери один правильный вариант. Термины занятия — [в справке](@vyzov-instrumentov).»
  - `options`:
    - { text: «Модель сама выполняет код на складе и приносит данные из базы», is_correct: false }
    - { text: «Модель понимает, что нужен внешний инструмент (например „склад“), формирует вызов, программа выполняет её и возвращает результат — модель отвечает по нему», is_correct: true }
    - { text: «Модель придумывает правдоподобный ответ, чтобы не звать склад», is_correct: false }
  - `explanation`: «Модель не выполняет код и не лезет в склад сама: она решает, какой инструмент вызвать,
    и формирует аргументы. Реальный инструмент (чтение файла склада, запрос к базе) выполняет программа,
...
- **Назначение:** теория-практика: студент в живом диалоге с Василичем-5.7 проверяет, что модель про
  живые позиции склада не знает (отвечает «из головы»), — мотивация для вызова инструментов. Проверка через `checkPrompt`.
- **Содержание:**
  - `task` (Markdown): «Не тяни резину, молодежь! Дежурный по цеху (занятие 4) отвечает по инструкции и по архиву (занятие 5),
    ...
    и почему без вызова инструмента точный список не получить. По готовности нажми „Проверить“.»
  ...
  - `checkPrompt`: «Оцени ответ студента: понял ли он, что модель отвечает „из головы“ и не имеет доступа
    к живым данным склада (позициям на сегодня), поэтому для точного списка нужен вызов инструмента „склад“
    (tool calling). Ответь „да“ или „нет“ с кратким пояснением.»
...
- **Содержание:**
  ...
  - `description` (Markdown): «Это живой склад — то, что будет читать инструмент „склад“. Скачай оба файла.
    В `sklad.csv` — позиции на сегодня (порошок, сода, цемент; а ещё сахар — но у него остаток 0,
    в наличии его нет, этикетку не делаем). Памятка — подсказка, как собирать вызов инструмента и проверять
    ответ. Всё понадобится на слайде 15.»
  - `files`:
    - { name: `sklad.csv`, description: «Живой склад: позиции (и остатки — для инструмента „склад“)» }
    - { name: `pamyatka-knopka-sklad.md`, description: «Памятка: как собрать вызов инструмента „склад“ и проверить ответ» }
...
- **Назначение:** подготовка к главной практике: что соберёт агент (скрипт с вызовом инструментов через
  OpenRouter: инструмент «склад» читает `sklad.csv`, модель сама решает вызвать) и этикетки для позиций
  в наличии; сравнение с ответом без инструмента (выдумка), секреты в `.env`, экономика токенов, проверка
...
«Вот что соберёт агент по твоим словам: Python-скрипт, который через API OpenRouter и **вызов инструментов**
отвечает на вопрос о реальных позициях склада. В коде описан инструмент „склад“: `sklad(tovar)`
читает по названию позиции `sklad.csv` и возвращает карточку (есть ли в наличии). На вопрос
смены модель сама решает, что нужен этот инструмент, программа его выполняет, результат возвращается модели —
...
Для наглядности соберём и наивный вариант: тот же вопрос **без** инструмента — модель ответит „из головы“.
Сравнишь — и увидишь разницу.

Сделать можно на платформе или в локальном opencode — как удобнее. Ключ OpenRouter — только в `.env`,
не в код и не в чат. Экономика: описание инструмента уходит в контекст вместе с запросом — это токены,
поэтому инструменты описываем коротко и только нужные. И главное: ответ сверяем с файлом склада,
...
- **Назначение:** главная практика-контрольная точка занятия: студент словами описывает агенту задачу —
  собрать скрипт с вызовом инструментов через OpenRouter (инструмент «склад» читает `sklad.csv`) + генерацию
...
- **Содержание:**
  ...
  - `task` (Markdown): «Чего сидишь, молодежь! Выбери, где сделаешь задание: **на платформе** — опиши задачу агенту здесь (в чате
    ...
    **1) „кнопка склад“:** через API OpenRouter и **вызов инструментов** отвечает на вопрос о реальных позициях
    склада: инструмент „склад“ — `sklad(tovar)` читает карточку из `sklad.csv` по названию позиции
    (папка задания); модель сама решает, когда её вызвать. Сделай и наивное сравнение: тот же вопрос
    **без** инструмента — модель ответит „из головы“.
    **2) этикетки:** читает `sklad.csv` и генерирует этикетки для позиций **в наличии** (порошок, сода,
    ...
    модели без инструмента и **проверь глазами каждую этикетку** (текст на картинках может „плыть“ — подправь
  ...
  - `checkPrompt`: «Оцени работу студента: собран ли скрипт с вызовом инструментов через OpenRouter (инструмент
    „склад“ читает `sklad.csv`), секреты — только в `.env`; студент получил реальный список позиций склада
    (порошок стиральный, сода, цемент), сравнил с ответом модели без инструмента; скрипт сгенерировал этикетки
...
## Видео: план ролика «Вызов инструментов: руки для модели»

> Технический ролик на Manim (ManimCE), `videos/manim/06-vyzov-instrumentov.py`. Движок — Manim, потому
> что ролик объясняет механизм (цикл вызова инструмента). Стиль и параметры — как в первом ролике
> занятия. 1920×1080, 30 fps, длительность ~98 с, фиксированный `seed`.

**Мысль ролика (одна):** для живых данных RAG не годится — нужен вызов инструмента; модель сама решает
вызвать инструмент, формирует вызов спец-токеном, обвязка выполняет скрипт и возвращает реальный
результат, модель отвечает по нему; без инструмента — «из головы», и это надо проверять.

| № | Время | Кадр | Что происходит | Надписи в кадре |
|---|-------|------|----------------|-----------------|
| 1 | 0–24 с | RAG не всегда подходит | Запрос летит в модель («ДУМАЮ...» мигает); из векторной базы валятся ордера, +/- подписи сваливаются кучей к Василичу; индикатор «КОНТЕКСТНОЕ ОКНО» переполняется (>100%, красный мигает, заливка уходит за край кадра), чипы «А ИНВЕНТАРИЗАЦИЯ?»... ; вместо ответа — мигающие ошибки | «RAG НЕ ВСЕГДА ПОДХОДИТ» |
| 2 | 24–34 с | Данные живые | ПОГОДА / НОВОСТИ / ПОКАЗАНИЯ ДАТЧИКОВ / ОСТАТКИ НА СКЛАДЕ зачёркиваются по одному | «ЖИВЫЕ ДАННЫЕ БЕРЁМ ВЫЗОВОМ ИНСТРУМЕНТА» |
| 3 | 34–88 с | Вызов инструмента под капотом | Колонки UI / ОБВЯЗКА (HARNESS) / МОДЕЛЬ (LLM): запрос + чипы системного промпта и инструмента; **карточка «обвязка добавляет промпт и описания инструментов»**; пакет летит в модель; ответ со спец-токенами (`<|plugin_call|> get_sklad({})`), **карточка «модель вызывает инструмент спец-тегом <|plugin_call|>»**; сегмент подсвечен; вертикальная стрелка к скрипту; скрипт вспыхивает зелёным, в UI «ЗАПУСК СКРИПТА...»; json-ответ → чип «РЕЗУЛЬТАТ РАБОТЫ СКРИПТА» в UI; стопка пяти чипов летит в модель копией; ответ под ней; **карточка «обвязка по тегу вызывает инструмент и передаёт результат модели»** | «ВЫЗОВ ИНСТРУМЕНТА ПОД КАПОТОМ» |
| 4 | 88–98 с | Вывод | Крупная финальная надпись | «ИНСТРУМЕНТ — РУКИ МОДЕЛИ: ВЫЗВАЛ → РЕАЛЬНЫЙ РЕЗУЛЬТАТ → ОТВЕТ ПО ДЕЛУ» |

**Что НЕ показываем:** развёрнутого сюжета с лицами — ролик технический, про цикл вызова. Сюжетная
рамка только в Василиче (меняет настроение). После ролика — слайд 9 (формальное объяснение вызова
инструментов).
...
| `vyzov-instrumentov` | 7, 9, 10 | «вызов инструментов (tool calling)» |
