...
def noise_cloud(center, radius_x=1.6, radius_y=1.1, n=90):
    """Пятно «шума»: случайные точки внутри эллипса (детерминировано по seed)."""
...
class IzTekstaKartinka(Scene):
    def construct(self):
        random.seed(config.seed if config.seed is not None else 6)
        self.prompt_into_model()
        self.shum_to_kartinka()
        self.etiketa()
        self.povtor()
        self.final_message()
    ...
    def prompt_into_model(self):
        ...
        self.play(FadeOut(prompt), FadeOut(arrow), FadeOut(note), run_time=0.4)
    ...
    def shum_to_kartinka(self):
        ...
        # стадии прояснения: точки сдвигаются в «мешок» с этикеткой
        mechok = RoundedRectangle(width=2.6, height=1.8, corner_radius=0.15,
                                  fill_color=DUST, fill_opacity=0.0,
                                  stroke_color=BEIGE, stroke_width=5)
    ...
    def etiketa(self):
        ...
        self.play(FadeIn(name), FadeIn(band), run_time=0.8)
        ...
        self.etiketa_group = (e, name, band, out)
