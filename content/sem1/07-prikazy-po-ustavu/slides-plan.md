...
````markdown
«По-взрослому, без сказок. **Промпт** — это текст, который мы отдаём модели. Как у приказа, у него есть
части:
...
Не все части обязательны и порядок свободный, но есть правило: **инструкцию лучше ставить в конец** —
тогда модель её выполняет, а не „дочитывает контекст“ вместо задания.
...
- **Содержание:**
  - `task` (Markdown): «Дежурный по цеху — сама модель, поэтому она лучше всех знает, как ей приказы давать.
...
- **Назначение:** разрядка после блока теории и перед главной практикой. Сквозная тема — результат
  проверяет человек.
- **Содержание:**
  ...
  - `description`: «Скажешь как попало — получишь как попало. Кандидаты для замены: `m01433` („умная мысль“
...
- **Содержание:**
  - `task` (Markdown): «Проведи серию экспериментов и запиши устав для дежурного.
