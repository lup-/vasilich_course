...
> он: „хватит, с запасом!“. А потом цех без порошка стоит. А мой старый учитель в школе вдолбил: считай по
> шагам — и у него всё было верно. А молодёжь как перескочит через действие — так и ошибка! Твою ж
> изоленту! Вот и черту скажем: рассуждай шаг за шагом. Прогони его на наших расчётах — запасы, логистика
> — и скажи мне, где он точнее считает, а где всё так же наобум!»
> — Прораб Василич
...
|---|-----|----------|--------------------|
| 1 | `image_group` | Комикс «Думающий ИИ» | 5 панелей: дежурный на вопрос «хватит ли порошка» отвечает наобум → Василич: «мой старый учитель в школе вдолбил: считай по шагам» → Алиса: попроси модель «рассуждать шаг за шагом» → задача: проверить на расчётах запасов и логистики |
| 2 | `rich_text` | «Считает по шагам» | Метафора от Алисы: учитель Василича в школе вдолбил считать по шагам — не сбивался; модель то же: «рассуждай шаг за шагом» — ответ точнее |
| 3 | `video` | «Думает вслух» | Классная комната: молодой Василич, учитель: «Сколько повторять?! Нужно делать по шагам, а не считать все в уме!» — мотивация для CoT |
...
- **Назначение:** завязка занятия: продолжение хука занятия 07 (Василич задумался про сложные приказы
  ...
  вспоминает учителя в школе, который вдолбил: считай по шагам — и у него всё было верно; Алиса объясняет,
  что модель можно попросить рассуждать по шагам; стажёру поручают проверить приёмы на заводских расчётах.
  См. `comic.md`, сцена 1.
- **Содержание:**
  - `title`: «Думающий ИИ»
  ...
  - `description`: «Аппаратная. Смена спрашивает дежурного: „Хватит ли порошка до поставки?“ — а он отвечает
    ...
    „Вот мой старый учитель в школе вдолбил: считай по шагам — и у него всё было верно. А молодёжь
    перескочит через действие — и ошибка!“ Алиса объясняет: модель можно попросить рассуждать вслух —
    «распиши ход решения по шагам». Стажёру поручают: прогнать дежурного на расчётах запасов и логистики
    и сказать, где он считает точно, а где наобум.»

### 2. rich_text — «Считает по шагам»

- **Назначение:** первое понятие от имени Алисы (п.7 шаблона), по лекции `chain_of_thought` курса-02:
  метафора «учитель Василича учил считать по шагам» для CoT. Одна мысль: попросить модель показать ход решения —
  ответ точнее.
- **Содержание** (`markdown`):
...
«Вот смотри, деточка. Учитель у Василича в школе учил: „двадцать три… двадцать четыре…“ — каждый шажок
...
А попросишь **рассуждать шаг за шагом** — она распишет ход решения, как учил учитель у Василича, и не собьётся.
...
- **Назначение:** наглядно показать, почему важно просить модель рассуждать по шагам: в классе учитель
  заставляет молодого Василича и одноклассников решать пример по действиям, а не в уме. Видео подводит к
  идее CoT — «подумай шаг за шагом».
- **Содержание:**
  - `title`: «Думает вслух»
  - `video_url`: `slide-03/08-dumaet-vslukh.mp4`
  - `video_type`: `direct`
  - `description`: «Видео: классная комната, молодой Василич за партой. Учитель ругает класс за неверно
    решённый пример: „Сколько повторять?! Нужно делать по шагам, а не считать все в уме!“. Этот момент
    становится основой для приёма Chain of Thought — просить модель рассуждать вслух.»
...
- **Содержание:**
  ...
  - `options`:
    ...
    - { text: «Подключить модели новую кнопку-инструмент», is_correct: false }
...
- **Содержание:**
  ...
  - `show_frames`: true
  - `description`: «В аппаратной на доске — таблица точности: „без рассуждений — мимо, по шагам — в яблочке,
    большинством — в яблочке“. Стажёр задаёт дежурному вопрос „хватит ли порошка“ — и тот расписывает:
    „425 − 216 = 209 кг“. Василич сверяет с бумагой склада: „ОСТАТОК: 209 КГ. Совпало! Теперь черт считает,
    как мой учитель в школе вдолбил!“. Алиса хвалит, но напоминает про проверку. Василич задумывается: „Раз умеет
