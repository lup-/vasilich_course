...
> «Над проходной слово намалевали — вода не берёт, мыло не берёт, и растворители из магазина — тоже!
> Дирекция чует: суперстойкая краска конкурента «ХимБытИскИнЛакоКраск». Лаборатория протокол составила — сделаем под неё
> свой растворитель и выпустим на рынок, чтоб неповадно было! Только каждую цифру сверь с базой:
...
«Интернет, деточка, — это огромный склад, а веб-API — дверь на него. Приходишь по адресу (URL),
...
Заказать данные агенту — формула: что нужно, откуда (URL), какой формат, как проверить.
...
«Рецептуру, деточка, никому на слово не доверишь. Для молекул источник правды — база PubChem:
миллионы веществ с формулой, массой и структурой. Агент „придумал“ вещество — сверяемся с PubChem.
...
Приём проверки: попроси агента получить официальный SMILES и массу по названию и сравнить со своим
...
Цифры оттуда вытаскивает PyMuPDF, и это переговоры: извлекли → посмотрели → уточнили.
...
и сверяем с протоколом глазами.
...
- **Содержание:**
  ...
  - `image_url`: `memes/images/m01447.jpg` (кандидат; картинку проверяет человек глазами — по правилам
    курса)
  - `description`: «„Вы должны уважать мнение других“ — „их мнение: 3² = 6“. Как и растворитель
    на проверке: вроде очевидно, а проверь по источнику. На заводе каждый вывод — сверен с PubChem
    и подкреплён DOI, а не „мнением“.»
