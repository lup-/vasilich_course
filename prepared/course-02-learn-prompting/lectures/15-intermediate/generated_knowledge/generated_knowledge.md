...
The idea behind the generated knowledge approach(@liu2021generated) is to ask the LLM to generate potentially useful information about a given question/prompt before generating a final response.
...
If we first ask LLM to generate facts about Congo and South Africa, we can then use that information to answer the question correctly. In theory, this is similar to CoT prompting, since we are effectively getting the LLM to generate intermediate reasoning in the form of related facts.

Let's start with the first step, _knowledge generation_. We can ask the LLM to generate facts about Congo and South Africa:
